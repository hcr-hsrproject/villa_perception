...
void Bounding_Box_dobject::ImageCallback(const sensor_msgs::ImageConstPtr& msg) {
	img = (*msg);
	received_image = true;
...
void Bounding_Box_dobject::yolo_detected_obj_callback(const tmc_yolo2_ros::DetectionsConstPtr &msg){
    ...
    for(size_t i = 0; i < objects.size(); i++){
        ...
        rect.insert( rect.end(), rect_temp.begin(), rect_temp.end() );
        // rect.push_back(rect_temp[0]);
        // rect.push_back(rect_temp[1]);
        // rect.push_back(rect_temp[2]);
        // rect.push_back(rect_temp[3]);

        // rect.push_back(rect_temp);
