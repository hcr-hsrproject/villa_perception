...
int main(int argc, char **argv)
{
	...
	Bounding_Box_dobject_nicolas bbh_obj;


  	// Create Subsribers
	bbh_obj.yolo_detectedObjects_sub = bbh_obj.node.subscribe<tmc_yolo2_ros::Detections>("yolo2_node/detections", 10, boost::bind(&Bounding_Box_dobject_nicolas::yolo_detected_obj_callback, &bbh_obj, _1));	
    bbh_obj.registered_cloud_sub = bbh_obj.node.subscribe<sensor_msgs::PointCloud2>("/hsrb/head_rgbd_sensor/depth_registered/rectified_points", 10, boost::bind(&Bounding_Box_dobject_nicolas::cloud_callback, &bbh_obj, _1));

	bbh_obj.prism_voxels_pub = bbh_obj.node.advertise<pcl::PointCloud<pcl::PointXYZRGB> >( "/prism_voxels", 0 );
	
	bbh_obj.Objects_points_pub = bbh_obj.node.advertise<pcl::PointCloud<pcl::PointXYZRGB> >( "/Objects_points", 0 );
	...
	bbh_obj.image_sub = bbh_obj.node.subscribe<sensor_msgs::Image>("/hsrb/head_rgbd_sensor/rgb/image_rect_color", 1000, boost::bind(&Bounding_Box_dobject_nicolas::ImageCallback, &bbh_obj, _1));
