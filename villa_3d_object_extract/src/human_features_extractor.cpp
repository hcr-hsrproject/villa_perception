...
void Bounding_Box_dobject::dn_extract_points(const sensor_msgs::ImageConstPtr& msg){
	if (boxes.size() > 0){
		...
		for (int b_i = 0; b_i < boxes.size(); b_i++){		
			...
			for (int m = 0; m < height; m++){
				for (int n = 0; n < width; n++){
			    	...
			    	red = red + cv_ptr->image[point_index].r;
			      	green = green + cv_ptr->image[point_index].g;
			      	blue = blue + cv_ptr->image[point_index].b;
