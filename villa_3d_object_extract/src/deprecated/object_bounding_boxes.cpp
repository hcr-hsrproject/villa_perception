...
#define CAMERA_PIXEL_HEIGHT 640
#define VOXEL_SIZE 0.05 //5cm voxels
#define CLUSTER_DIST_TOLERANCE 0.1
...
public:
	...
	void detected_obj_callback(const dn_object_detect::DetectedObjectsConstPtr &msg);
...
void Bounding_Box_dobject::detected_obj_callback(const dn_object_detect::DetectedObjectsConstPtr &msg){
	std::vector<dn_object_detect::ObjectInfo> objects;
	objects = msg->objects;

	std::string person;	person = "person";
	int number_of_objects = objects.size();

	boxes.clear();
	for (size_t i = 0; i < number_of_objects; i++){
//		if (objects[i].type.compare(person) == 0){
/*			std::cout << objects[i].type << " ";
			std::cout << objects[i].tl_x << " ";
			std::cout << objects[i].tl_y << " ";
			std::cout << objects[i].width << " ";
			std::cout << objects[i].height << std::endl;*/

			boxes.push_back(BoundingBox_Person_Desc( objects[i].tl_x,  objects[i].tl_y,  objects[i].width,  objects[i].height));
...
void Bounding_Box_dobject::dn_extract_points(const sensor_msgs::PointCloud2ConstPtr &msg){
	if (boxes.size() > 0){
		...
		for (int b_i = 0; b_i < boxes.size(); b_i++){		
			...
			int tl_index = (y*CAMERA_PIXEL_HEIGHT) + x;
			...
			for (int m = 0; m < height; m++){
				for (int n = 0; n < width; n++){
					int point_index = tl_index + (m*CAMERA_PIXEL_HEIGHT) + n;
				    // Prepare for copying
...
{
	ros::init(argc, argv, "node_people_bounding_boxes");

	Bounding_Box_dobject bbh_obj;

  	// Create Subsribers
	bbh_obj.detectedObjects_sub = bbh_obj.node.subscribe<dn_object_detect::DetectedObjects>("dn_object_detect/detected_objects", 10, boost::bind(&Bounding_Box_dobject::detected_obj_callback, &bbh_obj, _1));
