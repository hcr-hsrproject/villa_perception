...
class FaceDectector_Action(object):
	_feedback = face_recognition.msg.FaceRecogFeedback()
    _result = face_recognition.msg.FaceRecogResult()

	def __init__(self, name, wait=0.0):
		self._action_name = name
        self._as = actionlib.SimpleActionServer(self._action_name, face_recognition.msg.FaceRecogAction, execute_cb=self.execute_cb, auto_start = False)
        self._as.start()
	...
	def listener(self,goal,wait=0.0):
		rospy.spin()
...
if __name__ == '__main__':
	...
	Face_manager = FaceDectector_Action(rospy.get_name())
