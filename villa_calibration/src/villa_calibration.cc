...
#include "OptRTDistCostFunctor.h"
#include "RigidTrans.h"
#include "SingleCameraOptimizer.h"
#include "TransformShortcuts.h"
#include "Util.h"
#include "ZhangLinearEstimate.h"
...
int main(int argc, char **argv) {
    ...
    Size s(5, 7);
    ModelChessboard chessboard(s.height, s.width, 20.0);

    string modelName = "hand_fish";
    //string modelName = "head_fish";
    //string modelName = "head_rgbd";
    vector<string> files = listFiles("/home/justin/hsr_photos/" + modelName +
        "/");
    vector<Mat> images;
    vector<MatrixXd> camPoints;
    detectCorners(s, loadImages(files), images, camPoints);
    
    vector<MatrixXd> homographies = computeHomographies(chessboard, camPoints);
    
    ZhangLinearEstimate zhangLE(homographies);
    zhangLE.prettyPrint();

   vector<MatrixXd> optH = optimizeHomographies(
        chessboard.getModelCBH2D(), camPoints, homographies);

    vector<RigidTrans> rtoEs = extractRTs(zhangLE.getCameraIntrinsics(),
        optH);

    CameraOptimizationParameters cop(zhangLE.getCameraIntrinsics());
    SingleCameraOptimizer sco(
        cop, zhangLE.getCameraIntrinsics(), chessboard.getModelCBH3D());
    ...
    fs << "   cols:   3" << endl;
    fs << "   data:   [" <<
        sco.getKOpt().getAlpha() << ", " <<
        sco.getKOpt().getGamma() << ", " <<
        sco.getKOpt().getU0() << ", " <<
